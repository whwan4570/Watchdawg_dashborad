...
def display_cache_info(_, refresh_count):
    ...
    try:
        ...
        if conn:
            ...
            return [
                ...
                html.Div("📁 Source: crime_data_gold.db (SQLite)", className="small")
            ]
    except Exception as e:
    ...
    return html.Div([
        ...
        "No data loaded - check database file"
...
def init_category_filter(_):
    ...
    try:
        # Load a sample to get categories (no date filter needed for categories)
        df = load_all_data(start_date=None, end_date=None, limit=10000)
...
def init_neighborhood_filter(_):
    ...
    try:
        # Load a sample to get neighborhoods (no date filter needed for neighborhoods)
        df = load_all_data(start_date=None, end_date=None, limit=10000)
...
def display_date_info(date_data):
    ...
    try:
        # Query with date range for filtered count
        df_filtered = load_all_data(start_date=start_date, end_date=end_date)
...
def update_trend_chart(date_data, sort_order, metric):
    ...
    try:
        ...
        # Get data with date filter (memory efficient - only loads needed data)
        df = load_all_data(start_date=start_date, end_date=end_date)
...
def update_crime_type_chart(drill_level, selected_category, reset_counter, date_data):
    ...
    try:
        ...
        # Load data with date filter (memory efficient - only loads needed data)
        df = load_all_data(start_date=start_date, end_date=end_date)
...
def update_kpis(date_data):
    ...
    try:
        start_date = date_data.get("start_date") if date_data else None
        end_date = date_data.get("end_date") if date_data else None
        # For KPIs: use date-filtered data (memory efficient - only loads needed data)
        df_kpi = load_all_data(start_date=start_date, end_date=end_date)
        print(f"DEBUG KPI: Loaded {len(df_kpi)} records for KPIs")
...
def update_table(poly_geojson, hour_value, category_value, neighborhood_value, circle_coords, circle_radius, date_data):
    ...
    # 1) Load data with date filter (memory efficient - only loads needed data)
    try:
        df = load_all_data(start_date=start_date, end_date=end_date)
        print(f"DEBUG TABLE: Loaded {len(df)} rows with date filter ({start_date} to {end_date})")
...
if __name__ == "__main__":
    ...
    # Don't pre-warm data cache - load on demand for memory efficiency
    print("🔥 App starting, data will be loaded on demand (memory efficient).")
